...
Page({
  ...
  async loadData() {
    ...
    try {
      ...
      if (!signup) signup = await coachApi.createSignup({ meetId, eventId, teamId: await this.resolveTeamId(), signupType: '代表队', status: '草稿' });
      ...
      const statusText = { DRAFT: '草稿', REJECTED: '已驳回', SUBMITTED: '已提交', APPROVED: '已通过', LOCKED: '已锁定' }[signup.status] || signup.status || '';
      this.setData({ loading: false, signupId: signup.id, view: Object.assign({}, signup, { meetName: meet && meet.meetName, eventName: event && event.eventName, statusText }), athletes: selectedA, officials: selectedO, availableAthletes: (athletes || []).map((item) => Object.assign({}, item, { checked: selectedAIds.indexOf(Number(item.id)) >= 0, jerseyNo: (selectedA.find((row) => Number(row.id) === Number(item.id)) || {}).jerseyNo || '' })), availableOfficials: (officials || []).map((item) => Object.assign({}, item, { checked: selectedOIds.indexOf(Number(item.id)) >= 0 })) });
  ...
  async saveRoster() { try { const a = this.data.availableAthletes.filter((item) => item.checked).map((item, index) => ({ athleteId: item.id, jerseyNo: item.jerseyNo || null, captain: false, sortNo: index + 1 })); const o = this.data.availableOfficials.filter((item) => item.checked).map((item, index) => ({ officialId: item.id, sortNo: index + 1 })); await Promise.all([coachApi.saveSignupAthletes(this.data.signupId, a), coachApi.saveSignupOfficials(this.data.signupId, o)]); wx.showToast({ title: '名单已保存', icon: 'success' }); await this.loadData(); } catch (error) { this.showError(error.message); } },
  async submit() { try { await this.saveRoster(); await coachApi.submitSignup(this.data.signupId); wx.showToast({ title: '报名已提交', icon: 'success' }); await this.loadData(); } catch (error) { this.showError(error.message); } },
