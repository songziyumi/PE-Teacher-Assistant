...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Prepare remote course-selection concurrency test data.")
    ...
    parser.add_argument("--output-dir", default="temp/generated")
...
class RemoteClient:
    ...
    def get_text(self, path: str, headers: Optional[Dict[str, str]] = None) -> Tuple[str, str]:
    ...
    def get_bytes(self, path: str, headers: Optional[Dict[str, str]] = None) -> Tuple[bytes, str, Dict[str, str]]:
        req = urllib.request.Request(self._full_url(path), headers=headers or {})
        with self.web_opener.open(req, timeout=60) as resp:
            response_headers = {k: v for k, v in resp.headers.items()}
            return resp.read(), resp.geturl(), response_headers
    ...
    def login_web(self, username: str, password: str) -> None:
        ...
        response_html, final_url = self.post_form(
            "/login",
...
def generate_workbooks(
    ...
    return classes_path, students_path, records
...
def fetch_students_from_classes(
    client: RemoteClient,
    token: str,
    ...
    for class_row in class_rows:
        ...
        path = f"/api/admin/students?classId={class_id}&page=0&size={students_per_class + 20}"
        ...
        if len(matched) != students_per_class:
            raise RuntimeError(
                f"班级 {class_row['name']} 仅匹配到 {len(matched)} 名测试学生，预期 {students_per_class} 名"
            )
...
def save_event_students(client: RemoteClient, event_id: int, student_ids: List[int]) -> Dict[str, str]:
    ...
    fields: List[Tuple[str, str]] = [("_csrf", csrf)]
...
def parse_course_rows(html_text: str, event_id: int) -> Dict[str, Dict[str, object]]:
    ...
    for match in pattern.finditer(html_text):
        ...
        result[name] = {"id": int(match.group("course_id"))}
    return result
...
def activate_courses(client: RemoteClient, event_id: int, course_rows: Dict[str, Dict[str, object]]) -> None:
    ...
    for course in course_rows.values():
        client.post_form(
            f"/admin/courses/{event_id}/courses/{course['id']}/activate",
            [("_csrf", csrf)],
        )
...
def verify_student_access(client: RemoteClient, base_url: str, account: Dict[str, str]) -> Dict[str, object]:
    token = client.api_request(
        "POST",
        "/api/auth/login",
        json_body={"username": account["loginId"], "password": account["password"]},
    )["data"]["token"]
    ...
    return {
        "event": event_payload.get("data"),
        "courseCount": len(courses_payload.get("data") or []),
    }
...
def main() -> int:
    ...
    classes_path, students_path, _ = generate_workbooks(
    ...
    client.login_web(args.username, args.password)
    admin_token = client.api_login(args.username, args.password)
    ...
    expected_students = args.class_count * args.students_per_class
    ...
    sample_verification = verify_student_access(client, args.base_url, sample_account)
