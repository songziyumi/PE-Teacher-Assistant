...
- 当前目标：完成赛事系统 Web 管理端 MVP，并在浏览器联调前补齐关键回归测试
...
- [x] 当前本地可通过 Maven 编译
- [x] 关键赛事回归测试已具备持续扩展基础
- [ ] 建议新增一个联调前本地 tag 作为恢复点
- [ ] 如需远端备份，仍需先处理 Git 代理配置问题

## 8. 还需要你重点验证的事项

- [ ] 市级管理员、区县管理员、学校管理员三类账号的数据可见范围是否符合业务预期
- [ ] 报名提交、区县审核、市级审核、成绩发布的完整流程是否符合真实业务口径
- [ ] 赛事详情页四个分页签和操作按钮显示是否符合预期

## 9. 下一步建议顺序

- [ ] 跑一次赛事模块定向总回归并收口这一轮测试补强
- [ ] 打一个新的本地 tag 作为浏览器联调前恢复点
- [ ] 进行浏览器联调，整理页面异常与交互问题
