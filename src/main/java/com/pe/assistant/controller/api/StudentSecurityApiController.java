...
public class StudentSecurityApiController {

    private final CurrentUserService currentUserService;
    private final AccountEmailService accountEmailService;

    @GetMapping("/account-security")
    public ApiResponse<Map<String, Object>> accountSecurity() {
        StudentAccount account = currentUserService.getCurrentStudentAccount();
        return ApiResponse.ok(accountEmailService.buildStudentAccountSecurity(account));
    }
    ...
    public ApiResponse<String> updateNotifyToggle(@RequestBody NotifyToggleRequest request) {
        try {
            StudentAccount account = currentUserService.getCurrentStudentAccount();
            accountEmailService.updateStudentNotifyEnabled(account, request != null && Boolean.TRUE.equals(request.getEnabled()));
