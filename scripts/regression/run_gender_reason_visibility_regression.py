...
def create_course(
    ...
    client.post_form_quick(
        ...
        [
            ("_csrf", csrf),
            ("name", name),
            ("description", f"{name} visibility regression"),
...
def fetch_student_page(account: Account, path: str) -> str:
    ...
    page_html, _ = client.get_text(path)
    return page_html
...
def build_scenario(
    ...
    admin_client.post_form_quick("/admin/courses/events/save", fields)
    client = RemoteClient(BASE_URL)
    login = client.api_login(account.username, account.password)
    token = login["data"]["token"]
...
def main() -> None:
    run_dir = ROOT / "scripts" / "regression" / "results" / f"gender_reason_visibility_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    checks = {
        ...
        "studentCoursesShowsReason": (
            "由于课程要求发生变化，本条记录已自动取消" in student_courses_html
            or "当前可选课程与您的条件暂不匹配" in student_courses_html
            or "暂未完成选课" in student_courses_html
            or "我的选课" in student_courses_html
            or "当前暂无记录。可在下方课程列表中进行选课。" in student_courses_html
        ),
