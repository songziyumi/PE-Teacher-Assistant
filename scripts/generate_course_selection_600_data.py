...
COURSES = [
    CourseDef("篮球基础", "GLOBAL", 56),
    CourseDef("足球训练", "GLOBAL", 52),
    CourseDef("羽毛球提高", "GLOBAL", 58),
    CourseDef("排球技巧", "GLOBAL", 54),
    CourseDef("乒乓球进阶", "GLOBAL", 50),
    CourseDef("健美操", "GLOBAL", 48),
    CourseDef("武术入门", "GLOBAL", 60),
    CourseDef("田径提升", "PER_CLASS", 48, 4),
    CourseDef("跳绳挑战", "PER_CLASS", 48, 4),
]
...
def write_courses_csv(path: Path) -> None:
    lines = ["课程名称,名额模式,总名额,按班名额说明"]
    for course in COURSES:
        per_class_note = "" if course.capacity_mode == "GLOBAL" else f"每个行政班 {course.per_class_capacity} 人"
        lines.append(f"{course.name},{course.capacity_mode},{course.total_capacity},{per_class_note}")
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def write_sql(path: Path, admin_classes: list[dict[str, str]], preferences: list[tuple[str, str, int, str]]) -> None:
    course_values = ",\n".join(
        f"  ({sql_literal(course.name)}, {sql_literal(course.capacity_mode)}, {course.total_capacity})"
        for course in COURSES
    )
...
def write_readme(path: Path, preferences: list[tuple[str, str, int, str]]) -> None:
    ...
    lines = [
        ...
        f"- `course-selection-600.sql`：创建活动 `{EVENT_NAME}`、9 门课程、按班名额、活动参与名单，以及 600 名学生的第一/第二志愿。",
        "",
        "## 课程设置",
        "",
    ]
    ...
    for course in COURSES:
        if course.capacity_mode == "GLOBAL":
            lines.append(f"- {course.name}：全局名额 {course.total_capacity}。")
        else:
            lines.append(f"- {course.name}：按班级名额，每个行政班 {course.per_class_capacity} 人，总名额 {course.total_capacity}。")
